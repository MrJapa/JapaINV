...
#BUNGEE LAYER FONT
##############################
server = "blueworld-datacapture.database.windows.net"
database = "Inventory"
UID = "blueinventory"
passw = "Blue2021"
...
##!End!##
##############################
exitmain_startcom = lambda: [exit_main(), computers_screen()]
exitmain_startpho = lambda: [exit_main(), phones_screen()]
exitmain_starthar = lambda: [exit_main(), hardware_screen()]
exitmain_startnet = lambda: [exit_main(), network_screen()]
###!!####
exitcom_startmain = lambda: [exit_com(), main_screen()]
exitpho_startmain = lambda: [exit_pho(), main_screen()]
exithwd_startmain = lambda: [exit_hdw(), main_screen()]
exitnet_startmain = lambda: [exit_net(), main_screen()]
##############################
def login_screen():
    global signin
    signin = tk.Toplevel()
...
def main_screen():
    ...
    mainscreen.title("JapaINV")
    mainscreen.iconbitmap(icon)
    mainscreen.resizable(False,False)
    mainscreenbg = PhotoImage(file = bg_main)
    
    canvas_ms = Canvas(mainscreen,width=1366, height=768)
    canvas_ms.pack(fill = "both", expand=True)
    canvas_ms.create_image(0,0, image = mainscreenbg, anchor = "nw")

    computers = PhotoImage(file = "images/computers.png")
    computer_button = Button(text="", image=computers,highlightthickness=0,bd=0, command=exitmain_startcom)
    computer_canvas = canvas_ms.create_window(400,58,anchor="center",window=computer_button)

    phones = PhotoImage(file= "images/phones.png")
    phones_button = Button(text="", image = phones,highlightthickness=0,bd=0,command=exitmain_startpho)
    phones_canvas = canvas_ms.create_window(600,58,anchor="center",window=phones_button)

    hardware = PhotoImage(file= "images/hardware.png")
    hardware_button = Button(text="", image = hardware,highlightthickness=0,bd=0,command=exitmain_starthar)
    hardware_canvas = canvas_ms.create_window(800,58,anchor="center",window=hardware_button)

    network = PhotoImage(file= "images/network.png")
    network_button = Button(text="", image = network,highlightthickness=0,bd=0,command=exitmain_startnet)
    network_canvas = canvas_ms.create_window(1000,58,anchor="center",window=network_button)

    user_button = Button(mainscreen,text=login.get(),fg="white",bg="#00b5ff", font="Calibri 20 bold",highlightthickness=0,bd=0,command=sign_out)
    user_canvas = canvas_ms.create_window(1300,35,anchor="center",window=user_button)

    charts = PhotoImage(file= "images/charts.png")
    charts_button = Button(text="",image=charts,highlightthickness=0,bd=0,command=None)
    charts_canvas = canvas_ms.create_window(350,500,anchor="center",window=charts_button)
    ...
    databases_canvas = canvas_ms.create_window(950,500,anchor="center",window=databases_button)
...
def computers_screen():
    ...
    computersscreenbg = PhotoImage(file = bg_main)
    
    global canvas_cs
    canvas_cs = Canvas(computersscreen,width=1366, height=768)
...
def phones_screen():
    ...
    phonesscreenbg = PhotoImage(file = bg_main)
    
    global canvas_ps
    canvas_ps = Canvas(phonesscreen,width=1366, height=768)
    ...
    trvp.column(1, minwidth=100, width=100)
    trvp.column(2, minwidth=100, width=100)
    trvp.column(3, minwidth=100, width=100)
    trvp.column(4, minwidth=100, width=100)
...
def hardware_screen():
    ...
    hardwarescreenbg = PhotoImage(file = bg_main)
...
def network_screen():
    ...
    networkscreenbg = PhotoImage(file = bg_main)
...
def search():
    ...
    if len(rows) == 0:
        cursor.execute("SELECT COUNT (*) FROM Computers")
        rowcount = cursor.fetchone()[0]
        labelcount2.config(searchframe,text="Total records: %s" % rowcount)
    else:
        cursor.execute("SELECT COUNT (*) FROM Computers WHERE Model = (?) or [User] = (?)",ent.get(),ent.get())
        rowcount = cursor.fetchone()[0]
        labelcount2.config(searchframe,text="Total records: %s" % rowcount)
...
def search_hardware():
    ...
    if len(rows) == 0:
        ...
        cursor.execute("SELECT COUNT (*) FROM Hardware WHERE Type = (?) or [User] = (?) or Model = (?) or Manufacturer = (?)",enth.get(),enth.get(),enth.get(),enth.get())
        rowcount = cursor.fetchone()[0]
        labelcount2h.config(searchframeh,text="Total records: %s" % rowcount)
...
def search_network():
    ...
    if len(rows) == 0:
        ...
        cursor.execute("SELECT COUNT (*) FROM Network WHERE Type = (?) or Model = (?) or Manufacturer = (?) or IPAddress = (?)",entn.get(),entn.get(),entn.get(),entn.get())
        rowcount = cursor.fetchone()[0]
        labelcount2n.config(canvas_ps,text="Total records: %s" % rowcount)
#update network rows
...
def sign_up_screen():
    ...
    bg_signup = PhotoImage(file="images/create.png")
...
canvas_ml = Canvas(main_login, width=500, height=500)
